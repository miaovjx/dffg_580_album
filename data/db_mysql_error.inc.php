...
<?
	...
	if(isset($GLOBALS['_DSESSION']['zsyx114_user'])) {
		$errmsg .= "<b>User</b>: ".htmlspecialchars($GLOBALS['_DSESSION']['zsyx114_user'])."\n";
	}
	...
	if($GLOBALS['adminemail']) {
		$errlog = array();
		if(@$fp = fopen('./data/dberror.log', 'r')) {
		...
		if(!in_array($dberrno, $errlog)) {
			...
			if($GLOBALS['dbreport']) {
				...
				@sendmail($GLOBALS['adminemail'], 'MySQL Error Report',
						"There seems to have been a problem with the database of your PHP \n\n".
						strip_tags($errmsg)."\n\n".
						"Please check-up your MySQL server , similar errors will not be reported again in recent 24 hours\n".
